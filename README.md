...
1. Spout를 지원하는 미디어 서버 앱(예: Resolume)에서 Spout Output을 활성화합니다.
2. Roland FillKey Bridge를 실행합니다.
3. 사용할 Spout Sender를 선택합니다.
...
## Roland-FillKey-Bridge
Roland FillKey Bridge converts RGBA output from Spout-compatible media server App (such as Resolume) into a real-time Roland V-series (e.g., V-1-4K) INPUT 5 Fill+Key compatible video stream.

### Quick Start
1. Enable Spout Output in your Spout-compatible media server application (e.g. Resolume).
2. Launch Roland FillKey Bridge.
3. Select the desired Spout Sender.
...
- On icon – Spout connected
- Off icon – Spout disconnected
